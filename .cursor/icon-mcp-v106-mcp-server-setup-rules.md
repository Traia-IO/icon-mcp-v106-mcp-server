# ICON MCP v106 MCP Server Implementation Guide

You are working on implementing the ICON MCP v106 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: ICON MCP v106
- **Documentation**: []()
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_icon_mcp_v106",
      "get_icon_mcp_v106_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["icon mcp v106", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the ICON MCP v106 API with retry support. Example pattern:
...
def call_icon_mcp_v106_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call ICON MCP v106 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"ICON MCP v106 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_icon_mcp_v106(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search ICON MCP v106 for [specific data type].
    ...
    try:
        # The call_icon_mcp_v106_api function already has retry logic
        results = call_icon_mcp_v106_api(
...
```python
@mcp.tool()
async def get_icon_mcp_v106_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_icon_mcp_v106_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in ICON MCP v106.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_icon_mcp_v106(query="example", limit=5)
   ```
...
### 9. Common ICON MCP v106 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on ICON MCP v106 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the ICON MCP v106 documentation that you should implement:
...
- Check the ICON MCP v106 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make ICON MCP v106's capabilities easily accessible to AI agents through standardized MCP tools. 