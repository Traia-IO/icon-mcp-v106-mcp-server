...
# Script to build and run the ICON MCP v106 MCP Server locally in Docker
...
IMAGE_NAME="icon-mcp-v106-mcp-server"
CONTAINER_NAME="icon-mcp-v106-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running ICON MCP v106 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# ICON MCP v106 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ ICON MCP v106 MCP Server is running!${NC}"
echo
